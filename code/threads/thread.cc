...
Thread::Thread(char* threadName)
{
    name = threadName;
	UserId = getuid();
	ThreadId = iCurThreadId++;
	ThreadPriority = CreatePriority;		//set ThreadPriority when create new thread
...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) 
	{
		...
		currentThread->ThreadPriority = BlockedPriority;
...
//----------------------------------------------------------------------
//  GetInstance
//	this function will return a pointer of thread,if iMaxnum below 128,or 
//	return null;
//----------------------------------------------------------------------
Thread* Thread::GetInstance(char* debugName)
{
	if (iMaxThreadNum < ThreadNum)
	{
		Thread* pthread = new Thread(debugName);
		iMaxThreadNum++;
		return pthread;
	}
}
